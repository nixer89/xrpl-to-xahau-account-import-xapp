...
export class XahauWebsocket {
    ...
    mainNodes:string[] = ['wss://xahau.org','wss://xahau.network'];
    ...
    async getWebsocketMessage(componentname:string, command:any, newTestMode:boolean, retry?:boolean): Promise<any> {
        ...
        if(!this.websocketMap.get(componentname) || this.websocketMap.get(componentname).socket.closed) {
            ...
            let newWebsocket = webSocket(this.originalTestModeValue ? (this.testFirst ? this.testNodes[0] : this.testNodes[0]) : (this.mainFirst ? this.mainNodes[0] : this.mainNodes[1]));
